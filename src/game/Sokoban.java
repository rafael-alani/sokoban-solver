...
public class Sokoban {
	...
	private Board initBoard() {
		...
		switch (config.levelFormat) {
			case S4JL:
				result = Board.fromFileS4JL(config.levelFile, config.levelNumber);
				break;
			case SOK:
				result = Board.fromFileSok(config.levelFile, config.levelNumber);
				break;
		}
		result.validate();
		return board = result;
	 ...
	 * 
	 * Result of the game or games is going to be stored within
	 * {@link #getResults()}.
	 * Use {@link #getResults()}, {@link #getResult()} and {@link #getLastResult()}
	 * to obtain it/them.
	 * 
	 */
		...
		if (config.levelNumber >= 1) {
			...
			try {
				while (true) {
					...
					if (levelNumber != 1
							&& (getLastResult() == null || getLastResult().getResult() != SokobanResultType.VICTORY)) {
	...
	/**
	 * Runs Sokoban game according to the 'config'; method assumes the configuration
	 * is going to play single level only.
	 */
	...
	/**
	 * Runs Sokoban game according to the 'config'; method assumes the configuration
	 * is going to play one or more levels.
	 * If there are multiple levels to be played, the run will stop when agent fails
	 * to solve the level.
	 */
	...
	/**
	 * 'agent' will play (headless == simulation only) 'levelNumber' (1-based) level
	 * from file on 'levelFilePath' assuming 'levelFormat'.
	 ...
	 * @param levelFormat   expected format of the file (if it is null, it will be
	 *                      auto-determined)
	 * @param levelNumber   1-based; a level to be played
	 * @param timeoutMillis time given to the agent to solve every level;
	 *                      non-positive number == no timeout
	 * @param agent
	 * @return
	 */
	...
	/**
	 * 'agent' will play (visualized) 'levelNumber' (1-based) level from file on
	 * 'levelFilePath' assuming 'levelFormat'.
	 * An agent will be given 'timeoutMillis' time to solve the level.
	 * 
	 * @param id            id to be given to the config; may be null
	 * @param levelFilePath file to load the level from
	 ...
	 * @return
	 */
	public static SokobanResult playAgentLevel(String id, String levelFilePath, ELevelFormat levelFormat,
			int levelNumber, int timeoutMillis, IAgent agent) {
		// CREATE CONFIG
		SokobanConfig config = new SokobanConfig();
		if (id == null)
			id = determineId(agent);
		...
		if (!config.levelFile.exists() || !config.levelFile.isFile())
			throw new RuntimeException("Not a level file at '" + config.levelFile.getAbsolutePath()
					+ "'\nResolved from: " + levelFilePath);
		if (levelFormat == null)
			levelFormat = determineLevelFormat(config.levelFile.getName());
	 ...
	 * @return
	 */
	public static SokobanResult playAgentLevel(String id, String levelFilePath, int levelNumber, int timeoutMillis,
			IAgent agent) {
	 ...
	 * @param levelFilePath file to load the level from
	 * @param levelFormat   expected format of the file; if it is null, it will be
	 *                      auto-determined using
	 *                      {@link ELevelFormat#getExpectedLevelFormat(File)}
	 * @param levelNumber   1-based; a level to be played
	 * @param timeoutMillis time given to the agent to solve every level;
	 *                      non-positive number == no timeout
	 * @param agent
	 * @return
	 */
	public static List<SokobanResult> playAgentFile(String id, String levelFilePath, ELevelFormat levelFormat,
			int timeoutMillis, IAgent agent) {
		// CREATE CONFIG
		SokobanConfig config = new SokobanConfig();
		if (id == null)
			id = determineId(agent);
	...
	/**
	 * 'agent' will play (visualized) the given level number from the file on
	 * 'levelFilePath'.
	 * 
