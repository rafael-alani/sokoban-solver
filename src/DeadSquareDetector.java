...
public class DeadSquareDetector{
    public static boolean[][] detect(BoardCompact state) {
        ...
        // We do this because in the case that there is a box in between two walls but there is another space in two walls in the same row/column we don't act as there is a reward in the row
        // I like to call this space between two walls a closure
        // Then after detecting this closure and having all the tiles inside it we firstly always mark corners as dead because corners are always dead unless there is a box that needs to go there
        // In the case there is a reward inside the closure only corners will be marked
        // After having the extremes of the closure evaluated we need to check the inside tiles
        // Inside tiles are only marked if both extremes are corners and if there is a continuos wall in between them
        // Until now this works. I think that with massive boards it might take a while because of the amount of tiles that we quarry so im sure there is room for optimization
                ...
                if (CTile.isWall(state.tile(x, y))) {
                    ...
                    for (int tile : closures) {
                        if (!contains_goal) {
                            ...
                            if ((CTile.isWall(state.tile(x,tile-1)) || CTile.isWall(state.tile(x,tile+1))) && !CTile.forSomeBox(state.tile(x, tile))){ dead[x][tile] = true;}
                        }
                        c++;
                        old_tile = tile;
                    }
                ...
                if (CTile.isWall(state.tile(x, y))) {
                    ...
                    for (int tile : closures) {
                        if (!contains_goal) {
                            ...
                            if ((CTile.isWall(state.tile(tile-1,y)) || CTile.isWall(state.tile(tile+1,y))) && !CTile.forSomeBox(state.tile(tile, y))){ dead[tile][y] = true;}
                        }
                        c++;
                        old_tile = tile;
                    }
